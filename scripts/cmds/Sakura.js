...
module.exports = {
  config: {
    ...
    countDown: 5,
    role: 0,
    description: { en: "Get 6 random Sakura Haruno images." },
    category: "naruto",
    guide: { en: "/sakura - Get 6 random Sakura Haruno images" },
  },

  onStart: async function ({ api, event }) {
    try {
      const query = "Sakura Haruno"; // Fixed search query
      const count = 6;
      ...
      for (let i = 0; i < Math.min(count, shuffled.length); i++) {
        ...
        try {
          ...
          const imgPath = path.join(cacheDir, `sakura_${i + 1}.jpg`);
      ...
      await api.sendMessage(
        { body: `🌸 Here are 6 random Sakura Haruno images!`, attachment: attachments },
        event.threadID,
        event.messageID
      );
      ...
      console.error("Sakura Img Error:", err.message);
