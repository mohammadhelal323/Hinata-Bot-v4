...
module.exports = {
  config: {
    ...
    category: "naruto",
    guide: { en: "/kakashi - Get 6 Kakashi Hatake images" },
  },

  onStart: async function ({ api, event }) {
    try {
      const query = "Kakashi Hatake"; // Fixed search query
      const count = 6;
      ...
      for (let i = 0; i < Math.min(count, shuffled.length); i++) {
        ...
        try {
          ...
          const imgPath = path.join(cacheDir, `kakashi_${i + 1}.jpg`);
      ...
      await api.sendMessage(
        { body: `🌀😎 Here are 6 random Kakashi Hatake images!`, attachment: attachments },
        event.threadID,
        event.messageID
      );
      ...
      console.error("Kakashi Img Error:", err.message);
