...
module.exports = {
  config: {
    ...
    countDown: 5,
    role: 0,
    description: { en: "Get 6 random Rock Lee images." },
    category: "naruto",
    guide: { en: "/rocklee - Get 6 random Rock Lee images" },
  },

  onStart: async function ({ api, event }) {
    try {
      const query = "Cute rock lee anime photo"; // Fixed search query
      const count = 6;
      ...
      for (let i = 0; i < Math.min(count, shuffled.length); i++) {
        ...
        try {
          ...
          const imgPath = path.join(cacheDir, `rocklee_${i + 1}.jpg`);
      ...
      await api.sendMessage(
        { body: `💪 Here are 6 random Rock Lee images!`, attachment: attachments },
        event.threadID,
        event.messageID
      );
      ...
      console.error("RockLee Img Error:", err.message);
