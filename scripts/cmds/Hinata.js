...
module.exports = {
  config: {
    ...
    countDown: 5,
    role: 0,
    description: { en: "Get 6 random cute Hinata Hyuga anime-style images." },
    category: "naruto",
    guide: { en: "/hinata - Get 6 Hinata Hyuga anime-style images" },
  },

  onStart: async function ({ api, event }) {
    try {
      const query = "Cute Hinata Hyuga photo Anime style"; // Fixed search query
      const count = 6;
      ...
      for (let i = 0; i < Math.min(count, shuffled.length); i++) {
        ...
        try {
          ...
          const imgPath = path.join(cacheDir, `hinata_${i + 1}.jpg`);
      ...
      await api.sendMessage(
        { body: `💮 Here are 6 random cute Hinata Hyuga anime-style images!`, attachment: attachments },
        event.threadID,
        event.messageID
      );
      ...
      console.error("Hinata Img Error:", err.message);
