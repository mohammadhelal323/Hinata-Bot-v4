...
module.exports = {
  config: {
    ...
    countDown: 5,
    role: 0,
    description: { en: "Get a random funny meme image." },
    category: "meme",
    guide: { en: "/meme - Get 1 random funny meme image" },
  },

  onStart: async function ({ api, event }) {
    try {
      const query = "Funny Meme"; // Fixed search query
      const count = 1;
      ...
      const imgPath = path.join(cacheDir, `meme.jpg`);
      await fs.writeFile(imgPath, imgRes.data);

      await api.sendMessage(
        { body: `😂 Here's a random meme!`, attachment: fs.createReadStream(imgPath) },
        event.threadID,
        event.messageID
      );
      ...
      console.error("Meme Img Error:", err.message);
